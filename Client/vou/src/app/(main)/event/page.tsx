...
const Page = () => {
	...
	const fetchEvents = useCallback(async () => {
		...
		setError(null);
		try {
			const response = await EventService.getHotEvents(searchTerm, pageNumber, pageSize);
	...
	return (
		<div className={styles.eventPage}>
			...
			<div className="d-flex align-items-center justify-content-center mb-3">
				<div style={{fontWeight: "800", fontSize: "28px"}}>Hot Events</div>
				<IconEvent height="25px" />
			</div>

			<div className={styles.eventList}>
				{filteredEvents.length > 0 ? (
					filteredEvents.map(event => (
						<EventCard key={event[0].id} event={event} />
					))
				) : (
